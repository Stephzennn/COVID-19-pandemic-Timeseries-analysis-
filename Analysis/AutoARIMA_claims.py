...
for state in states:
    print(f"AUTO-SELECTING OPTIMAL ORDER: {state}")
    ...
    print(f"\n OPTIMAL ORDER FOR {state}:")
    print(f"  Order (p,d,q): {auto_model.order}")
    print(f"  Seasonal (P,D,Q,s): {auto_model.seasonal_order}")
    print(f"  AIC: {auto_model.aic():.0f}")
    print(f"  BIC: {auto_model.bic():.0f}")
    
    ARIMAX_RESULTS_AUTO[state] = {
